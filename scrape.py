...
QUERY = '久川颯 はーちゃん'  # 検索ワード
LIMIT_DL_NUM = 1000               # ダウンロード数の上限
SAVE_DIR = 'G:\OneDrive\powder\scrape\hayate\q_久川颯_はーちゃん'  # 出力フォルダへのパス（フォルダがない場合は自動生成する）
FILE_NAME = ''                       # ファイル名（ファイル名の後ろに０からの連番と拡張子が付く）
TIMEOUT = 10                     # 要素検索のタイムアウト（秒）
ACCESS_WAIT = 1                  # アクセスする間隔（秒）
RETRY_NUM = 3                    # リトライ回数（クリック、requests）
DRIVER_PATH = 'driver/chromedriver.exe'        # chromedriver.exeへのパス
...
while count < LIMIT_DL_NUM and count > count_before:
